  ...
  try {
    ...
    if (knownEnough) {
      ...
      const enrichPrompt = isDa
        ? `Vin: ${wineDesc}\nBrug din viden om DENNE specifikke vin (den konkrete cuvée + årgang) til at returnere et JSON-objekt med felterne:\n- grape: druetype/er — hvis blend, skriv alle druer med procentfordeling fx "Sangiovese 85%, Cabernet Sauvignon 15%"; hvis ukendt, tom string\n- region: korrekt region/appellation\n- land: land\n- about: 2-3 sætninger om DENNE vin, ikke huset generelt. Prioritér det vin-specifikke: vinmarken/appellationen, drueblandingen, vinens stil (fylde, tannin/syre, nøgle-aromaer) og et klassisk mad-match. Nævn kun producenten kort hvis det tilføjer noget. Skriv på dansk. VIGTIGT: opfind ALDRIG konkrete facts. Kender du ikke den præcise cuvée, så beskriv appellationen + druen + den typiske stil for denne vintype fra dette område — men gæt aldrig på specifikke detaljer. Hvis helt ukendt, tom string.\nReturner kun JSON.`
        : `Wine: ${wineDesc}\nUse your knowledge of THIS specific wine (the exact cuvée + vintage) to return a JSON object with:\n- grape: grape variety/varieties — if blend, list with percentages e.g. "Sangiovese 85%, Cabernet Sauvignon 15%"; if unknown, empty string\n- region: correct region/appellation\n- land: country\n- about: 2-3 sentences about THIS wine, not the house in general. Prioritise the wine-specific: the vineyard/appellation, the grape blend, the wine's style (body, tannin/acidity, key aromas) and a classic food pairing. Mention the producer only briefly if it adds context. IMPORTANT: NEVER invent concrete facts. If you don't know the exact cuvée, describe the appellation + grape + the typical style for this wine type from this area — but never guess specific details. If entirely unknown, empty string.\nReturn only JSON.`;
      try {
        const enriched = await callClaude({ model: WINE_MODEL, maxTokens: 500, system: enrichSystem, content: enrichPrompt });
  ...
  try {
    ...
    if (readableIdxs.length) {
      ...
      const batchPrompt = isDa
        ? `Her er ${readableIdxs.length} vine fra et lineup. Brug din viden om DEN specifikke vin (konkrete cuvée + årgang) til at returnere et JSON-array med præcis ${readableIdxs.length} elementer i SAMME RÆKKEFØLGE. Hvert element: { "grape": "druetype med % hvis blend, tom string hvis ukendt", "region": "korrekt region/appellation", "land": "land", "about": "2-3 sætninger på dansk om DENNE vin (ikke huset generelt): vinmarken/appellationen, drueblandingen, stilen (fylde, tannin/syre, aroma) og et klassisk mad-match. Opfind ALDRIG konkrete facts — kender du ikke cuvéen, beskriv appellation + drue + typisk stil; gæt aldrig. Tom string hvis helt ukendt" }. Returner KUN JSON-array.\n\nVine:\n${wineList}`
        : `Here are ${readableIdxs.length} wines from a lineup. Use your knowledge of THE specific wine (exact cuvée + vintage) to return a JSON array with exactly ${readableIdxs.length} elements in THE SAME ORDER. Each element: { "grape": "grape variety with % if blend, empty string if unknown", "region": "correct region/appellation", "land": "country", "about": "2-3 sentences about THIS wine (not the house in general): the vineyard/appellation, the grape blend, the style (body, tannin/acidity, aroma) and a classic food pairing. NEVER invent concrete facts — if you don't know the cuvée, describe appellation + grape + typical style; never guess. Empty string if entirely unknown" }. Return ONLY a JSON array.\n\nWines:\n${wineList}`;
      try {
        const enriched = await callClaude({ model: WINE_MODEL, maxTokens: Math.min(400 * readableIdxs.length, 4000), system: enrichSystem, content: batchPrompt });
